...
const WomensClothingPage = () => {
    ...
    useEffect(() => {
        const fetchWomens = async () => {
            try {
                ...
                if (!response.ok) {
                    throw new Error('Failed to fetch Womens products');
                }
                const data = await response.json();
                setProducts(data);
            } catch (error) {
                console.log(error, "Error while fetching Womens products");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div>
            <Navbar />
            <h1 className="text-3xl font-bold text-center my-2 bg-blue-400 text-white p-4">Women's Products</h1>
            <div>
